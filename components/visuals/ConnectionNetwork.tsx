  ...
  useEffect(() => {
    ...
    const build = () => {
      nodes = Array.from({ length: COUNT }, (_, i) => {
        const accent = i % 16 === 0; // a handful of green accent nodes
        return {
          x: Math.random() * w,
          y: Math.random() * h,
    ...
    const draw = () => {
      ...
      // Many points (countries); connect each to its neighbours so the mesh
      // stays legible and light instead of a solid full-mesh blob.
      const md = Math.min(w, h) * 0.22;
        ...
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < md) {
            ...
            ctx.strokeStyle = accent
              ? `rgba(10,138,63,${0.14 + k * 0.34})`
              : `rgba(120,120,128,${0.06 + k * 0.22})`;
            ctx.lineWidth = accent ? 0.9 : 0.6;
            ctx.beginPath();
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.stroke();
      ...
      for (const n of nodes) {
        ...
        ctx.fillStyle = n.accent ? "#0a8a3f" : "#aeaeb5";
        ctx.fill();
      }
    };
